...
#include <list>
#include <vector>
#include <fstream>//calvin
#include <openssl/rc4.h>//calvin
#include <openssl/evp.h>//calvin
#include "common.h"
#include "fdcache.h"
#include "s3fs.h"
#include "s3fs_util.h"
...
//-----------------------------------
//Calvin Bojanowski Encryption
//-----------------------------------
void calEncrypt(int fd){//calvin encryption
	...
	char char_key[] = "pass";
	unsigned char generated[16];//for evp 
	RC4_KEY key;
...
if (EVP_BytesToKey(EVP_rc4(),EVP_sha256(),NULL,(const unsigned char*)char_key,strlen(char_key),1,generated,NULL) < 0){
	...
	RC4(&key, buffer_size, (const unsigned char*) input, output);//encrypt
/*
	RC4_set_key(&key,strlen(char_key), (const unsigned char *)char_key);
	RC4(&key, buffer_size, (const unsigned char*) input, output);//encrypt
		*/
	lseek(fd, 0, SEEK_SET);

	if (write(fd, output, buffer_size) == -1) {
...
{
/* ofstream test;
test.open("//home//socks//Documents//4420_project//log.txt",ofstream::out | ofstream::app);
test << "entering CacheFileStat::Open()\n";//calvin bojanowski
test.close();//calvin*/
...
/* ofstream test;
test.open("//home//socks//Documents//4420_project//log.txt",ofstream::out | ofstream::app);
test << "entering FdEntity::RowFlush\n";//calvin bojanowski
test.close();//calvin*/
  int result = 0;
